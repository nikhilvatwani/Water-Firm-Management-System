...
<script>
    function clearAmount(ref,orderid,pending){
        ...
        if(!isNaN(value)){
            if(value<=pending){
                $.ajax({
                    url : 'http://localhost/advanced_old/backend/web/index.php?r=orders/clearcane&orderid='+orderid+'&value='+value,
                    dataType: "json",
                ...
                $(error).html('entered no. of canes is greater than pending');
            }

        }else{
            $(error).html('enter valid number of canes');
        }
...
<div class="pending-cane-form">
    ...
    ?>
        <tr>
            ...
            <td><?= $value['cane_pending'] ?></td>
            <td><?php $content = '<div class="form-group">
                                  <label for="usr">Number of Canes : </label>
...
echo PopoverX::widget([
    ...
    'content' => $content,
    'footer' => Html::button('Submit', ['class'=>'btn btn-sm btn-primary','id'=>'unique-'.$value['o_id'],'onclick'=>'clearAmount(this,'.$value['o_id'].','.$value['cane_pending'].')']),
